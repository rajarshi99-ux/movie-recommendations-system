...
class MovieRecommender:
    ...
    def __init__(
        ...
        self.model_mode = os.getenv("CINE_MODEL_MODE", "semantic_hybrid")
        if self.model_mode in ["semantic_only", "semantic_hybrid"]:
    ...
    def recommend(self, movie_title: str, n: int = 10) -> list:
        ...
        if self.model_mode in ["semantic_only", "semantic_hybrid"]:
            ...
            distances = 1.0 - all_sims[indices]  # convert sim to distance format purely for compat
        else:
            # TF-IDF search
        ...
        for dist, idx in zip(distances, indices):
            ...
            if self.model_mode == "tfidf_baseline":
                final_score = content_sim
            elif self.model_mode == "existing_hybrid":
                final_score = (0.70 * content_sim) + (0.20 * norm_rating) + (0.10 * norm_pop)
            elif self.model_mode == "semantic_only":
                final_score = content_sim
            elif self.model_mode == "semantic_hybrid":
                ...
                final_score = (0.60 * content_sim) + (0.15 * g_sim) + (0.10 * norm_rating) + (0.05 * norm_pop) + (0.10 * f_sig)
            else:
                final_score = (0.70 * content_sim) + (0.20 * norm_rating) + (0.10 * norm_pop)
            ...
            if self.model_mode in ["semantic_only", "semantic_hybrid"]:
                explanation = "Recommended because its plot and themes are semantically similar."
                if len(matching_genres) > 0:
                    explanation += f" Shares {len(matching_genres)} similar genres including {', '.join(list(matching_genres)[:3])}."
            else:
            ...
            result_dict["matching_keywords"] = top_keywords if self.model_mode not in ["semantic_only", "semantic_hybrid"] else []
            if self.model_mode == "semantic_hybrid":
